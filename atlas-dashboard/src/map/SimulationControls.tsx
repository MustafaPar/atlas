...
export default function SimulationControls({
  ...
  return (
    <div className="pt-3 border-t border-gray-100 space-y-2">
      ...
      {/* Arrived at pickup — prominent call to action */}
      {phase === 'at_pickup' && (
        <div className="rounded-lg bg-amber-50 border border-amber-200 px-3 py-2.5 space-y-2">
          <p className="text-xs font-semibold text-amber-800">📍 Arrived at pickup</p>
          <p className="text-xs text-amber-700">Confirm pickup to continue to delivery.</p>
          <button
            onClick={onConfirmPickup}
            disabled={apiLoading}
            className="w-full px-3 py-2 text-xs font-semibold bg-amber-500 text-white rounded-md hover:bg-amber-600 disabled:opacity-50 transition-colors"
          >
      ...
      {/* Arrived at dropoff — prominent call to action */}
      {phase === 'at_delivery' && (
        <div className="rounded-lg bg-green-50 border border-green-200 px-3 py-2.5 space-y-2">
          <p className="text-xs font-semibold text-green-800">🏁 Arrived at dropoff</p>
          <p className="text-xs text-green-700">Confirm delivery to complete this order.</p>
          <button
            onClick={onConfirmDelivery}
            disabled={apiLoading}
            className="w-full px-3 py-2 text-xs font-semibold bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50 transition-colors"
          >
